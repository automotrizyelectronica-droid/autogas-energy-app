...
# IMPORTANTE: Este correo debe ser el dueño de la carpeta y de los 2TB
...
def subir_foto_con_cuota(file_bytes, nombre):
    try:
        ...
        # 2. TRASPASAR PROPIEDAD A TU CUENTA (Para usar tus 2TB)
        permiso = {
            'type': 'user',
            'role': 'owner',
        ...
        # Esta es la línea que elimina el error 403
...
def generar_pdf(reg):
    ...
    pdf.cell(0, 10, "REPORTE TÉCNICO", 1, 1, 'C')
...
PAQUETES = {
    ...
    "D": ["Limpieza Inyectores", "Obturador", "Sensores", "Filtros", "Scan"],
    "E": ["Full Inyectores Gas/Gaso", "Regulacion", "Scan"],
    "F": ["Reductor Gas", "Calibracion", "Scan"]
}
...
if st.session_state.view == 'inicio':
    ...
    if st.session_state.step == 1:
        ...
        if st.button("✅ GUARDAR"):
            with st.spinner("Procesando fotos y datos..."):
                ids = [subir_foto_con_cuota(f.getvalue(), f"{d['placa']}_{datetime.now().strftime('%H%M%S')}.jpg") for f in fotos] if fotos else []
        ...
        if hist:
            ...
            for r in reversed(hist):
                with st.expander(f"Servicio {r['fecha']}"):
                    st.download_button("Descargar PDF", generar_pdf(r), f"Rep_{r['placa']}.pdf", key=f"{r['fecha']}_{r['km']}")
